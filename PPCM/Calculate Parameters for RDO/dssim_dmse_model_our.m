function [Thetanew, Theta, actual_dssim_our, estimate_dssim_our,rho_curr] = dssim_dmse_model_our(img1,img2,rho_prev,quantization_step,scale)
...
%
xcore = [-3, 0, 3; -10, 0, 10; -3, 0, 3];
xweights = conv2(img1,xcore,'valid')./16;
ycore = [-3, -10, -3; 0, 0, 0; 3, 10, 3];
yweights = conv2(img1,ycore,'valid')./16;
allweights = sqrt(xweights.^2+yweights.^2);
ThetaWeight = blkproc_li(allweights,ctu_size/f,@mean2);
ThetaWeight=zscore(ThetaWeight)/3+1;
% ThetaWeight = ThetaWeight./sum(sum(ThetaWeight)).*numel(ThetaWeight);

ThetaWeight=max(0.01,ThetaWeight);
cur_dmse = blkproc_li(blk_mse_curr, ctu_size/f/cu_size, @mean2);

Theta = Theta.*ThetaWeight;
%
estimate_dssim_our = cur_dmse.*Theta; % 

Thetanew = ThetaWeight;
